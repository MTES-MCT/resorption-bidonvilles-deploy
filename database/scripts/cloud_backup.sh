...
docker run --rm -i -v rb_database_backups:/data -v ${RCLONE_CONFIG_PATH}:/config/rclone/rclone.conf --user $(id -u):$(id -g) rclone/rclone copy --progress --error-on-no-transfer /data/$BACKUP_FILE_NAME secret:/
RCLONE_RESULT=$?
